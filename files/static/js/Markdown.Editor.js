...
(function () {
	...
	function PanelCollection(postfix) {
		this.buttonBar = postfix.find(".wmd-button-bar");
		this.preview = postfix.find(".wmd-preview");
		this.input = postfix.find(".wmd-input");
	};
	...
	util.addEvent = function (elem, event, listener) {
		if (elem[0].attachEvent) {
			// IE only.  The "on" is mandatory.
			elem[0].attachEvent("on" + event, listener);
		}
		else {
			// Other browsers.
			elem[0].addEventListener(event, listener, false);
		}
	};
	...
	function UndoManager(callback, panels) {
		...
		var setEventHandlers = function () {
			...
			var handlePaste = function () {
				if (uaSniffed.isIE || (inputStateObj && inputStateObj.text != panels.input.val())) {
					if (timer == undefined) {
						mode = "paste";
						saveState();
						refreshState();
	...
	function TextareaState(panels, isInitialState) {
		...
		this.init = function () {
			...
			if (!isInitialState && doc.activeElement && doc.activeElement !== inputArea[0]) { // this happens when tabbing out of the input box
				return;
			}

			this.setInputAreaSelectionStartEnd();
			this.scrollTop = inputArea[0].scrollTop;
			if (!this.text && inputArea[0].selectionStart || inputArea[0].selectionStart === 0) {
				this.text = inputArea[0].value;
			}
		...
		this.setInputAreaSelection = function () {
			...
			if (inputArea[0].selectionStart !== undefined && !uaSniffed.isOpera) {
				...
				if (doc.activeElement && doc.activeElement !== inputArea[0]) {
					return;
				}

				inputArea[0].focus();
				var range = inputArea[0].createTextRange();
				range.moveStart("character", -inputArea[0].value.length);
				range.moveEnd("character", -inputArea[0].value.length);
		...
		this.setInputAreaSelectionStartEnd = function () {
			...
			if (!panels.ieCachedRange && (inputArea[0].selectionStart || inputArea[0].selectionStart === 0)) {

				stateObj.start = inputArea[0].selectionStart;
				stateObj.end = inputArea[0].selectionEnd;
			}
			else if (doc.selection) {

				stateObj.text = util.fixEolChars(inputArea[0].value);
				...
				var inputText = util.fixEolChars(inputArea[0].value);
				...
				var len = stateObj.text.length - util.fixEolChars(inputArea[0].value).length;
		...
		this.restore = function () {
			...
			if (stateObj.text != undefined && stateObj.text != inputArea[0].value) {
				inputArea[0].value = stateObj.text;
			}
			this.setInputAreaSelection();
			inputArea[0].scrollTop = stateObj.scrollTop;
		};
	...
	function PreviewManager(converter, panels, previewRefreshCallback) {
		...
		var setPanelScrollTops = function () {
			if (panels.preview) {
				panels.preview[0].scrollTop = (panels.preview[0].scrollHeight - panels.preview[0].clientHeight) * getScaleFactor(panels.preview[0]);
			}
		};
		...
		var nonSuckyBrowserPreviewSet = function (text) {
			panels.preview[0].innerHTML = text;
		}
		...
		var pushPreviewHtml = function (text) {
			...
			var emptyTop = position.getTop(panels.input[0]) - getDocScrollTop();
			...
			var fullTop = position.getTop(panels.input[0]) - getDocScrollTop();
	...
	ui.prompt = function (title, text, defaultInputText, callback) {
		...
		var retval = window.prompt(text, defaultInputText);
		if (retval){
			callback(retval);
		}
		return false;
	...
	function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions) {
		...
		function doClick(button) {
          ...
          previewManager.refresh();
				...
				button.textOp(chunks, function(){ return false; });
		...
		function makeSpritedButtonRow() {
			...
			// note: this function has been changed quite a bit
			// the main diff being is that it no longer generates buttons, instead
			...
			buttons.redo = {
				execute : function (manager) { if (manager) manager.redo(); }
			};
			buttons.undo = {
				execute : function (manager) { if (manager) manager.undo(); }
			};
